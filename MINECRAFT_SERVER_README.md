...
- `/spawn` - Телепортироваться на безопасную позицию (если застряли в воздухе)
...
- ✅ Чат между игроками
- ✅ Система команд (`/help`, `/list`, `/time`, `/spawn`)
- ✅ Автоматическая проверка и исправление позиции спавна (предотвращает зависание в небе)
...
### Игрок зависает в небе (мир не прогружается)
Если игрок застрял в воздухе и мир не загружается:
- Сервер автоматически проверяет позицию игрока после спавна и телепортирует на безопасную позицию
- Если проблема сохраняется, используйте команду `/spawn` в игре для ручной телепортации
- Убедитесь, что чанки загружаются (проверьте логи: `pm2 logs minecraft-server`)
- Если проблема повторяется, попробуйте удалить мир и перезапустить сервер (см. раздел "Мир пустой")
